...
unsigned int spotter_init (struct GMT_CTRL *GMT, char *file, struct EULER **p, bool flowline, bool total_out, bool invert, double *t_max) {
	...
	e = gmt_M_memory (GMT, NULL, n_alloc, struct EULER);
	...
	while (gmt_fgets (GMT, buffer, GMT_BUFSIZ, fp) != NULL) { /* Expects lon lat t0 t1 ccw-angle */
		...
		if (GPlates) {
			...
			if (gmt_M_is_zero (t)) continue;	/* Not a rotation */
		...
		last_t = e[i].t_start;
		//if (e[i].omega == 0.0) continue;	/* skip null rotations [2016/10/18 PW: No, since GPlates sometimes contain null rotations between stable plates] */
		...
		if (i == n_alloc) {
			n_alloc <<= 1;
			e = gmt_M_memory (GMT, e, n_alloc, struct EULER);
		}
	}
	gmt_fclose (GMT, fp);
	...
	if (n < n_alloc) e = gmt_M_memory (GMT, e, n, struct EULER);
...
int spotter_hotspot_init (struct GMT_CTRL *GMT, char *file, bool geocentric, struct HOTSPOT **p) {
	...
	while (gmt_fgets (GMT, buffer, GMT_BUFSIZ, fp) != NULL) {
		...
		if (i == n_alloc) {
			n_alloc <<= 1;
			e = gmt_M_memory (GMT, e, n_alloc, struct HOTSPOT);
		}
	}
	gmt_fclose (GMT, fp);
	if (i < n_alloc) e = gmt_M_memory (GMT, e, i, struct HOTSPOT);
	*p = e;
...
unsigned int spotter_backtrack (struct GMT_CTRL *GMT, double xp[], double yp[], double tp[], unsigned int np, struct EULER p[], unsigned int ns, double d_km, double t_zero, unsigned int time_flag, double wesn[], double **c) {
	...
	for (i = 0; i < np; i++) {
		...
		if (path) {
			...
			if (kk == n_alloc) {
				n_alloc <<= 1;
				track = gmt_M_memory (GMT, track, n_alloc, double);
			}
		}
		nn = 0;
	...
	if (path) {
		track = gmt_M_memory (GMT, track, kk, double);
		*c = track;
		return (kk);
	}
...
void spotter_stages_to_total (struct GMT_CTRL *GMT, struct EULER p[], unsigned int n, bool finite_rates, bool stage_rates) {
	...
	for (stage = 0; stage < n; stage++) {
		...
		gmt_M_memcpy (R_old, R_young, 9, double);							/* Set R_old = R_young for next time around */
...
void spotter_add_rotations (struct GMT_CTRL *GMT, struct EULER a[], int n_a_in, struct EULER b[], int n_b_in, struct EULER *c[], unsigned int *n_c) {
	...
	t = gmt_M_memory (GMT, NULL, n_a + n_b, double);
	...
	b2 = gmt_M_memory (GMT, NULL, n_k, struct EULER);
	a2 = gmt_M_memory (GMT, NULL, n_k, struct EULER);
	c2 = gmt_M_memory (GMT, NULL, n_k, struct EULER);
	...
	for (i = 0; i < n_k; i++) {	/* Add each pair of rotations */
		...
		if (a2[i].has_cov && b2[i].has_cov) {	/* May compute combined covariance matrix, assuming khats = 1 */
			...
			if (sign_a < 0.0)
				...
				gmt_M_memcpy (Ca, a2[i].C, 9, double);
			if (sign_b < 0.0)
				spotter_cov_of_inverse (GMT, &b2[i], Cb);
			else
				gmt_M_memcpy (Cb, b2[i].C, 9, double);
...
bool spotter_conf_ellipse (struct GMT_CTRL *GMT, double lon, double lat, double t, struct EULER *p, unsigned int np, char flag, bool forward, double out[]) {
	...
	if (!forward) {		/* Rotate the point into the present */
		gmt_M_memcpy (cov, p[k].C, 9, double);	/* The rotation's covarience matrix */
	}
...
unsigned int spotter_confregion_radial (struct GMT_CTRL *GMT, double alpha, struct EULER *p, double **X, double **Y) {
	...
	gmt_M_memset (t, SPOTTER_N_FINE_STEPS+1, double);
	gmt_M_memset (t_inner, SPOTTER_N_FINE_STEPS+1, double);
	...
	*/
	...
	for (i = 0; i < na; i++) {
		...
		if (got_it) {	/* Got a valid point, now compute geographical coordinates */
			...
			if (n == n_alloc) {
				n_alloc <<= 1;
				lon = gmt_M_memory (GMT, lon, n_alloc, double);
				lat = gmt_M_memory (GMT, lat, n_alloc, double);
			}
		}
		else {
...
unsigned int spotter_confregion_ortho (struct GMT_CTRL *GMT, double alpha, struct EULER *p, double **X, double **Y) {
	...
	lon = gmt_M_memory (GMT, NULL, SPOTTER_N_STEPS, double);
	lat = gmt_M_memory (GMT, NULL, SPOTTER_N_STEPS, double);
	...
	for (i = 0; i < SPOTTER_N_STEPS; i++) {
		...
		gmtlib_get_point_from_r_az (GMT, p->lon, p->lat, dr_dist, azimuth, &lon[i], &lat[i]);
#ifdef DEBUG
...
void spotter_ellipsoid_normal (struct GMT_CTRL *GMT, double X[3], double L[3], double c, double N[3]) {
	...
	if (gmt_M_is_zero (X[GMT_Z])) {	/* Normal vector lies entirely in (x-y) plane */
		if (gmt_M_is_zero (X[GMT_Y])) {	/* Special case where N is aligned with x-axis */
